...
function normalizeYesNo(value) {
    ...
    if (['evet', 'yes', 'y', 'true', '1'].indexOf(v) >= 0) return true;
    if (['hayır', 'hayir', 'no', 'n', 'false', '0'].indexOf(v) >= 0) return false;
...
function yesNoCount(rows, fieldName) {
    var yes = 0;
    var no = 0;

    for (var i = 0; i < rows.length; i++) {
        var r = rows[i].raw || rows[i];
        var v = normalizeYesNo(r[fieldName]);
...
function renderGuestInsights(rows) {
    var el = pickEl('#guestInsights');
    if (!el) return;

    var prev = yesNoCount(rows, 'previousStay');
    var ret = yesNoCount(rows, 'willReturn');
    var rec = yesNoCount(rows, 'wouldRecommend');

    var commentCount = 0;
    for (var i = 0; i < rows.length; i++) {
        var txt = String((rows[i].raw || rows[i]).generalComments || '').trim();
        if (txt !== '') commentCount++;
    }

    var prevPct = percent(prev.yes, prev.total);
    var retPct = percent(ret.yes, ret.total);
...
function renderCommentFeed(rows) {
    ...
    for (var i = 0; i < rows.length; i++) {
        var r = rows[i].raw || rows[i];
        var txt = String(r.generalComments || '').trim();

        if (txt !== '') {
            list.push({
                name: r.fullName || 'Misafir',
                text: txt,
                staff: r.praisedStaff || '',
                date: r.tarih || r.date || '',
                returnText: r.willReturn,
                recText: r.wouldRecommend
            });
        }
    }

    if (list.length === 0) {
        el.innerHTML = '<p style="color:#999; text-align:center; padding:20px;">Yorum yok</p>';
        return;
    }

    var html = '';
    for (var j = 0; j < Math.min(8, list.length); j++) {
        var item = list[j];
...
function normalizeRecord(item) {
    ...
    return {
        ...
        previousStay: firstExistingValue(item, ['previousStay']) || '',
        ...
        willReturn: firstExistingValue(item, ['willReturn']) || '',
        wouldRecommend: firstExistingValue(item, ['wouldRecommend']) || '',
