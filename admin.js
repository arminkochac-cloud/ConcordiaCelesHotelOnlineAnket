...
function renderDashboard(data) {
    ...
    document.getElementById('countryChart').innerHTML = `
        ...
        <div class="bar-row"><div class="bar-label">Türkiye</div><div class="bar-area"><div class="bar-fill bg-blue" style="width:30%">30%</div></div></div>
    `;
    ...
    for (var i = 0; i < Math.min(5, data.length); i++) {
        tbody.innerHTML += '<tr><td>' + (data[i].date || '-') + '</td><td>' + (data[i].fullName || 'Misafir') + '</td><td>' + (data[i].roomNumber || '-') + '</td><td>' + (data[i].nationality || '-') + '</td><td>85/100</td></tr>';
    }
...
function renderDeptDetail() {
    ...
    el.innerHTML = '<div class="bar-row"><div class="bar-label">Hizmet Kalitesi</div><div class="bar-area"><div class="bar-fill bg-green" style="width:85%">85%</div></div></div>';
}
